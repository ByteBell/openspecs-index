...
If you believe you have found a security issue in OpenSpecs, please report it privately. **Do not open a public GitHub issue for security problems.**
...
- Theoretical issues without a reproducible impact on OpenSpecs.
- DDoS or volumetric reports against `bytebell.ai` web properties (this policy covers the OSS code only).

## Recognition

OpenSpecs is licensed under **AGPL-3.0-only with an additional non-commercial clause**. We do not run a paid bug bounty programme. With your permission, we will credit you in [CHANGELOG.md](CHANGELOG.md) and the release notes when a reported issue is fixed.
