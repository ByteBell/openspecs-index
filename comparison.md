# How every code-graph MCP tool works, and why Open-IR saves 80% on tokens
...
Open-IR builds a Neo4j graph too, so the architecture sounds similar on the surface. The difference is in what we put on each node, and how the nodes are connected to each other across repositories.
